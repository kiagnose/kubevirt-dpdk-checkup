...
Running [DPDK](https://core.dpdk.org/doc/) workloads on OpenShift Virtualization VMs requires careful configuration steps, both on the cluster and guest levels. However, by following the guidelines provided in this article, users can achieve lower latency and higher throughput for packet processing in user space
...
- [Deploy](https://docs.openshift.com/container-platform/4.12/virt/install/installing-virt-cli.html) OpenShift Virtualization plugin.tain
- [Create](https://docs.openshift.com/container-platform/4.8/applications/projects/working-with-projects.html) a project on the cluster where the VM will run.
...
*Note*: In this article an [Intel X710](https://www.dell.com/en-us/shop/intel-x710-quad-port-10gb-direct-attach-sfp-converged-network-adapter-full-height/apd/540-bbiw/networking#tabs_section) Network Adapter was used.
...
Doing so will result in the creation of a [network-attachment-definition](https://docs.openshift.com/container-platform/4.12/virt/virtual_machines/vm_networking/virt-attaching-vm-multiple-networks.html) that will be used by the OpenShift Virtualization VM in the next steps.
...
Doing so will create a [runtimeClass](https://kubernetes.io/docs/concepts/containers/runtime-class/) that would be used by the OpenShift Virtualization VM in the next steps.

In order to configure the cluster according to these steps please follow this OCP documentation.

## OpenShift Virtualization VM configuration
...
...
```
Note: In order to more easily identify and configure the SR-IOV interface inside the guest, NIC’s guest PCIAddress on the VM spec is set to 0000:07:00.0.

#### CPU pinning

To allow pinning a specific CPU quantity to the VM spec, the cpu snippet should look like this:
```yaml
apiVersion: kubevirt.io/v1
kind: VirtualMachine
spec:
...
- These annotations will be copied to the Pod running the OpenShift Virtualization VM, and along with the runtimeClassName set by Kubevirt, will configure the VM for DPDK performance.
...
```
Note: In this snippet we request 8 1Gi sized Hugepages.
